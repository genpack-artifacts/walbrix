...
def configure(ini):
    ...
    if installer_cmdline_exists(kernel_cmdline) or ini.getboolean("walbrix", "installer", fallback=False):
        os.makedirs(dropin_dir, exist_ok=True)
        with open(dropin, "w") as f:
            f.write("[Service]\nExecStart=\nExecStart=weston-deferred --modules=systemd-notify.so --shell=kiosk-shell.so --continue-without-input --config=/etc/xdg/weston/installer.ini\n")
    else:
